...
const PostDetails = ({ match: { params: { id } } }) => {
  ...
  // Fetch the post from state
  const post = useSelector(state => state?.post);
  const { loading, postDetails, appErr, serverErr, isDeletedRedirection } = post;
  ...
  if(isDeletedRedirection) return <Redirect to='/posts' />;
  return (
    <>
      { loading ? <div className="h-screen"><LoadingComponent /></div> : appErr || serverErr ? <h1 className="h-screen text-red-400 text-xl text-center">{serverErr} {appErr}</h1>: <section className="py-20 2xl:py-40 bg-gray-800 overflow-hidden">
          ...
          <div className="max-w-2xl mx-auto text-center">
            ...
            <div className="max-w-xl mx-auto">
              <p className="mb-6 text-left  text-xl text-gray-200">
                ...
                { isCreatedBy || userAuth?.isAdmin ? <p className="flex">
        ...
        { userAuth ? <AddComment postId={postDetails?._id} /> : null }
