...
import * as Yup from "yup"; // Validations
...
const Register = () => {
  ...
  // console.log(Object.entries(registered?.errors));
  // console.log(Object.values(registered?.errors));
  const errors = registered?.errors ? Object.values(registered?.errors) : [];
  // console.log(errors);
  // Redirect to login page
  if(registered && !registered.errors) return <Redirect to='/login' />
              ...
              <div className="px-6 lg:px-20 py-12 lg:py-24 bg-gray-600 rounded-lg">
                <form onSubmit={formik.handleSubmit}>
                  <h3 className="mb-10 text-2xl text-white font-bold font-heading">
                    ...
                    {/* {errors} */}
                    {errors.length > 0 ? errors?.map(error => (<div className="text-red-500">{error?.message}</div>)) : null}
                  </h3>
                  {/* First name */}
