...
const ResetPassword = (props) => {
  ...
  const { passwordReset, loading, appErr, serverErr, userAuth } = users;

  // Redirect to login page after 5 seconds of successfully reset password
  useEffect(() => {
    setTimeout(() => {
      // if (passwordReset) props.history.push("/login");
      if(passwordReset) props.history.push({ pathname: "/login" });
    }, 5000);
  }, [passwordReset]);
