...
export default function UpdatePassword() {
  ...
  const { loading, appErr, serverErr, profileUpdateRedirection, userAuth } = user;

  // Redirect
  if(profileUpdateRedirection) return <Redirect to={`/profile/${userAuth?._id}`} />
  ...
  return (
    <div className="min-h-screen bg-gray-700  flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        {appErr || serverErr ? <h2 className="text-center text-red-400 pt-2"><p>{serverErr} {appErr}</p></h2> : null}
      </div>
