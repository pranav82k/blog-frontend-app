...
import { logoutAsyncUserAction } from "../../../redux/slices/users/usersSlices";
...
export default function AccountVerified({
  ...
  const { loading, appErr, serverErr, verified } =
    accountVerification;

  return (
    <>
      {loading ? <LoadingComponent /> : verified ? (
        <div className="flex justify-center items-center min-h-screen bg-gray-400">
          <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
            ...
            <div className="mt-5 sm:mt-6">
              <button
                onClick={() => dispatch(logoutAsyncUserAction())}
                type="button"
        ...
        <>
          {appErr || serverErr ? <h2 className="text-center bg-gray-100 text-red-600">{serverErr} {appErr}</h2> : null}
