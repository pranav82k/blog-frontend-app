...
import { userProfileAction, userFollowAction, unFollowUserAction } from "../../../redux/slices/users/usersSlices";
import DateFormatter from "../../../utlis/DateFormatter";
import LoadingComponent from "../../../utlis/LoadingComponent";

export default function Profile({ computedMatch: { params: { id }}}) {
  //dispatch
  const dispatch = useDispatch();

  // Get the data from store
  const user = useSelector(state => state?.users);
  const { profileLoading, profileAppErr, profileServerErr, profile, userAuth, followed, unfollowed } = user;
  ...
  // check if the profile and loggedIn User are same or not
  const isLoginUser = userAuth?._id.toString() === profile?._id.toString();

  return (
    <>
      <div className="min-h-screen bg-green-600 justify-center items-center">
        { profileLoading ? <LoadingComponent/> : profileAppErr || profileServerErr ? <h2 className="text-yellow-400 text-2xl">{profileServerErr} {profileAppErr}</h2> : !profile ? <h2 className="text-yellow-400 text-2xl">No Profile Found</h2> : (
                ...
                <article>
                      ...
                      <div className="-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5">
                        ...
                        <div className="mt-6 sm:flex-1 sm:min-w-0 sm:flex sm:items-center sm:justify-end sm:space-x-6 sm:pb-1">
                          ...
                          <div className="mt-6 flex flex-col justify-stretch space-y-3 sm:flex-row sm:space-y-0 sm:space-x-4">
                            ...
                            { !isLoginUser ? (
                              <div>
                                { profile?.followers?.find(follwerUserId => follwerUserId === userAuth?._id) ? (<button
                                  onClick={() =>
                                ...
                                >
                                  ...
                                  <>
                                  <button
                                    onClick={() => dispatch(userFollowAction(profile?._id))}
                                    type="button"
                                    className="inline-flex justify-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
                                  >
                                    <HeartIcon
                  ...
                  <div className="flex justify-center place-items-start flex-wrap  md:mb-0">
                    <div className="w-full md:w-1/3 px-4 mb-4 md:mb-0">
                      ...
                      {/* Who view my post */}
                      <ul className="">
                        { profile?.viewedBy?.length <= 0 ? <h2 className="text-center text-lg text-yellow-400">No Viewer</h2> : profile?.viewedBy?.map(viewer => (
                          <li key={viewer?._id}>
                    ...
                    <div className="w-full md:w-2/3 px-4 mb-4 md:mb-0">
                      ...
                      {/* Loop here */}
                      { profile?.posts?.length <= 0 ? <h2 className="text-center text-xl">No post found</h2> : profile?.posts?.map(post => (
