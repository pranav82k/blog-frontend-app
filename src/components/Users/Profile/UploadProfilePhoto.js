...
export default function UploadProfilePhoto() {
  ...
  const { loading, appErr, serverErr, profilePhotoRedirection, userAuth } = user;

  if(profilePhotoRedirection) return <Redirect to={`profile/${userAuth?._id}`} />
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        {/* Displya err here */}
        { appErr || serverErr ? <h2 className="text-center text-xl text-red-600">{serverErr} {appErr}</h2> : null}
      </div>
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-6">
                    ...
                    <div className="container">
                      <div { ...getRootProps({
                        ...
                        onDrop: (event) => event.stopPropagation() }) }>
                          <input { ...getInputProps() } />
                          <p className='text-gray-800 text-md cursor-pointer text-center'>Click here to upload profile photo</p>
                        </div>
                    </div>
                  )
                }}
            ...
            <div>
              {loading ? (
                <button
                disabled
                className="inline-flex justify-center w-full px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-white bg-gray-500"
              >
                <UploadIcon
