...
import { sendMailAction } from "../../../redux/slices/emails/emailSlices";
...
const SendEmail = ({ location: { state } }) => {
  ...
  const { loading, appErr, serverErr, isRedirectOnMailSent } = mail;
  
  // Redirection
  if(isRedirectOnMailSent) return <Redirect to={`/profile/${state?.id}`} />;
  return (
